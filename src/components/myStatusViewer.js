...
export async function viewMyStatus() {
  ...
  if (!currentUser) {
    ...
    if (typeof showNotification === "function") {
      showNotification(
        "Vous devez être connecté pour voir vos statuts",
        "error"
      );
    }
    return;
...
function createMyStatusViewer(statusList, currentUser) {
  ...
  const modal = createElement("div", {
    class:
      "bg-white rounded-xl w-full max-w-md mx-4 max-h-[85vh] overflow-hidden shadow-2xl",
  });
  ...
  const header = createElement("div", {
    class: "bg-gradient-to-r from-green-300 to-green-300 p-6 text-white",
  });
  ...
  const avatar = createElement("div", {
    class:
      "w-12 h-12 bg-white bg-opacity-20 rounded-full flex items-center justify-center text-white mr-4 text-lg font-bold",
  });
  ...
  subtitle.textContent = `${statusList.length} statut${
    statusList.length > 1 ? "s" : ""
  }`;
  ...
  const closeButton = createElement("button", {
    class:
      "text-white hover:bg-white hover:bg-opacity-20 p-2 rounded-full transition-colors ml-4",
  });
  ...
  statusList.forEach((status, index) => {
    const statusItem = createElement("div", {
      class:
        "flex items-center p-4 hover:bg-gray-50 transition-colors border-b border-gray-100 last:border-b-0",
    });

    // Preview du statut
    const preview = createElement("div", {
      class:
        "w-16 h-16 rounded-xl mr-4 flex items-center justify-center overflow-hidden shadow-md cursor-pointer",
    });
    ...
    try {
      if (status.type === "text") {
        ...
        const previewText =
          status.content && status.content.length > 15
        ...
        img.onerror = () => {
          ...
          preview.className =
            "w-16 h-16 rounded-xl mr-4 flex items-center justify-center shadow-md cursor-pointer bg-gray-100";
        };
    ...
    metaElement.textContent = `${timeAgo} • ${viewsCount} vue${
      viewsCount > 1 ? "s" : ""
    }`;
    ...
    const viewBtn = createElement("button", {
      class:
        "text-green-500 hover:bg-green-50 p-3 rounded-full transition-colors",
  ...
  const addButton = createElement("button", {
    class:
      "w-full bg-gradient-to-r from-green-500 to-green-600 text-white py-3 rounded-xl hover:from-green-600 hover:to-green-700 transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105",
  });
  addButton.innerHTML =
    '<i class="fas fa-plus mr-2"></i>Ajouter un nouveau statut';
...
function deleteStatusConfirm(statusId, statusElement, subtitle, content) {
  try {
    ...
    if (success) {
      ...
      subtitle.textContent = `${remainingCount} statut${
        remainingCount > 1 ? "s" : ""
      }`;
...
function viewSingleStatus(status) {
  ...
  if (status.type === "text") {
    statusContent = createElement("div", {
      class:
        "w-full max-w-sm h-96 rounded-xl flex items-center justify-center text-white font-bold text-xl p-8 text-center shadow-2xl",
  ...
  const closeBtn = createElement("button", {
    class:
      "absolute top-4 right-4 text-white bg-black bg-opacity-50 hover:bg-opacity-70 p-3 rounded-full transition-colors z-10",
  });
...
function showMyStatusOptions() {
  ...
  const modal = createElement("div", {
    class:
      "bg-white rounded-xl w-full max-w-sm mx-4 p-8 shadow-2xl text-center",
  });
  ...
  description.textContent =
    "Vous n'avez pas encore créé de statut. Créez votre premier statut maintenant !";

  const createButton = createElement("button", {
    class:
      "w-full bg-[#33415c] text-white py-3 rounded-xl hover:bg-[#2b3547] transition-all duration-200 font-semibold shadow-lg hover:shadow-xl transform hover:scale-105 mb-3",
  });
