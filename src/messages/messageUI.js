...
export function createMessageBubble({ text, timestamp, status, isSent, attachments = [] }) {
  ...
  if (attachments.length > 0) {
    const attachmentsContainer = createElement("div", {
  ...
  if (isSent) {
    const statusIcon = createElement("i", {
      class: `fas fa-${
        status === 'sent' ? 'check' : 
        status === 'delivered' ? 'check-double' : 
        status === 'read' ? 'check-double text-blue-400' : ''
      } ml-1`
    });
